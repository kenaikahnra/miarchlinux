...
#Obtener nombre particiones
pBoot=fdisk -l /dev/$TARGET | grep '^/dev' | cut -d' ' -f1 | awk 'NR == 1'
pSwap=fdisk -l /dev/$TARGET | grep '^/dev' | cut -d' ' -f1 | awk 'NR == 2'
pRoot=fdisk -l /dev/$TARGET | grep '^/dev' | cut -d' ' -f1 | awk 'NR == 3'
pHome=fdisk -l /dev/$TARGET | grep '^/dev' | cut -d' ' -f1 | awk 'NR == 4'
export pBoot
